...
void EXTI4_IRQHandler(void)         //Interrupt frequency at 1KHz
{
    if(EXTI_GetITStatus(EXTI_Line4) != RESET)
    {
        EXTI_ClearFlag(EXTI_Line4);
        EXTI_ClearITPendingBit(EXTI_Line4);
        // Read origin sensor data
        LostCounterFeed(GetLostCounter(LOST_COUNTER_INDEX_IMU));   //feed(clear) the IMU ERROR Count
        isMPU6050_is_DRY = 1;   //mpu6050 interrupt flag
        GetPitchYawGxGyGz();		//Read attitude data, data is continous
																//读取姿态数据,数据已经处理成连续方式
    }
